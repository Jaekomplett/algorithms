...
function newChickenRecipe(stuffArr, choiceNum) {
  ...
  for (let i = 0; i < stuffArr.length; i++){
    const rotten = String(stuffArr[i])
    .split('')
    .filter(el => el === '0');

    if(rotten.length <= 2) {
      fresh.push(stuffArr[i]);
    }
  }
  fresh.sort((a, b) => a - b);
  // 조합 및 요소는 작은 숫자 -> 큰 숫자로 정렬합니다.
  if(fresh.length < choiceNum || fresh.length === 0) return []; 
  // 만약, 주어진 재료 모두 사용할 수 없다면 빈 배열[]을 반환해야 합니다.
  // 만약, 사용할 수 있는 재료가 choiceNum보다 작다면 빈 배열[] 을 반환해야 합니다.

  let result = [];
  function recur(arr, stack, n){
    if(n === 0){
      result.push(stack);
      return;
    }
    ...
    for (let i = 0; i < arr.length; i++){
      ...
      arrSlice.splice(i, 1);
      recur(arrSlice, stack.concat(arr[i]), n - 1)
      }
    }
    recur(fresh, [], choiceNum);
    return result;
}
