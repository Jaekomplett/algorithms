...
const powerSet = function (str) {
  let arr = str.split('').sort(); //문자열 분리 후, 정렬
  let result = ['']; 

  //result에 string을 추가한다.
  let putIn = (string, result) => { 
  ...
  for(let i = 0; i < arr.length; i++){ //배열 전체를 순회하고
    if(!result.includes(arr[i])){      //중복 value가 아니면 문자열 추가
      putIn(arr[i], result)
    }
  }
